...
public class StringMatrixRotation {
    public static void main(String[] args) {
        ...
        int colMaxLength = Integer.MIN_VALUE;

        while (!input.equals("END")) {
            int currentLength = input.length();
            if (currentLength > colMaxLength) {
                colMaxLength = currentLength;
            }
            wordList.add(input);
            input = scanner.nextLine();
        }
        int rows = wordList.size();
        int cols = colMaxLength;
        ...
        String angleToRotate = command.split("[()]")[1];
        int angle = Integer.parseInt(angleToRotate);
